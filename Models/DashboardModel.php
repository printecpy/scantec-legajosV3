...
class DashboardModel extends Mysql
{
    ...
    private function condicionPdfFinalDisponible(string $alias = 'l'): string
    {
        return "EXISTS (
                    SELECT 1
                    FROM unirpdf up
    ...
    public function selectCantArchivosDoc()
    {
        ...
        $sql = "SELECT DATE_FORMAT(cl.fecha_creacion, '%Y-%m-%d') AS fecha_indexado,
                ...
                WHERE cl.fecha_creacion >= CURDATE() - INTERVAL 15 DAY
                  AND (u.id_rol IS NULL OR u.id_rol <> 1)
    ...
    {
        $scope = $this->construirFiltroLegajos('cfg_legajo', $tiposPermitidos, $idUsuario, $soloPropios);
        $condicionPdfFinal = $this->condicionPdfFinalDisponible('cfg_legajo');
        $sql = "SELECT COUNT(*) AS cant_legajos_proceso
                FROM cfg_legajo
                WHERE (estado IN ('borrador', 'activo', 'completado')
                   OR (estado = 'finalizado' AND NOT $condicionPdfFinal))" . $scope['sql'];
        $res = $this->select($sql, $scope['params']);
        return $res;
    }
    ...
    public function selectLegajosCompletados(array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $sql = "SELECT COUNT(*) AS cant_legajos_completados
                FROM cfg_legajo
                WHERE estado = 'generado'" . $scope['sql'];
        $res = $this->select($sql, $scope['params']);
        return $res;
    }
    ...
    public function selectLegajosPorTipo(array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $condicionPdfFinal = $this->condicionPdfFinalDisponible('l');
        $sql = "SELECT
                ...
                WHERE (l.estado = 'generado'
                   OR (l.estado = 'finalizado' AND $condicionPdfFinal))" . $scope['sql'] . "
    ...
    public function selectLegajosPorUsuario(array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $sql = "SELECT
                ...
                WHERE l.estado = 'verificado'
                  AND (u.id_rol IS NULL OR u.id_rol <> 1)" . $scope['sql'] . "
    ...
    public function selectProductividadSolicitudesPorUsuario(int $periodoCantidad = 8, string $periodoUnidad = 'WEEK', array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $condicionPdfFinal = $this->condicionPdfFinalDisponible('l');
        $sql = "SELECT
                    productividad.nombre_usuario,
                    productividad.cantidad_proceso,
                    productividad.cantidad_completado,
                    productividad.cantidad_verificado
                FROM (
                    SELECT
                        COALESCE(u.nombre, 'Sin usuario') AS nombre_usuario,
                        SUM(CASE WHEN l.estado IN ('borrador', 'activo', 'completado') OR (l.estado = 'finalizado' AND NOT $condicionPdfFinal) THEN 1 ELSE 0 END) AS cantidad_proceso,
                        SUM(CASE WHEN l.estado = 'generado' OR (l.estado = 'finalizado' AND $condicionPdfFinal) THEN 1 ELSE 0 END) AS cantidad_completado,
                    ...
                    WHERE COALESCE(tl.requiere_nro_solicitud, 0) = 1
                      AND DATE(l.fecha_creacion) >= DATE_SUB(CURDATE(), INTERVAL ? " . $periodoUnidad . ")
                      AND (u.id_rol IS NULL OR u.id_rol <> 1)" . $scope['sql'] . "
                    GROUP BY l.id_usuario, u.nombre
                ) productividad
                WHERE (productividad.cantidad_proceso + productividad.cantidad_completado + productividad.cantidad_verificado) > 0
                ORDER BY (productividad.cantidad_proceso + productividad.cantidad_completado + productividad.cantidad_verificado) DESC,
                         productividad.nombre_usuario ASC;";
    ...
    public function selectLegajosArmadosPorFechaUsuario(int $periodoCantidad = 1, string $periodoUnidad = 'WEEK', array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $sql = "SELECT
                ...
                WHERE DATE(l.fecha_creacion) >= DATE_SUB(CURDATE(), INTERVAL ? " . $periodoUnidad . ")
                  ...
                  AND l.estado IN ('finalizado', 'verificado', 'cerrado', 'aprobado')
                  AND (ua.id_rol IS NULL OR ua.id_rol <> 1)" . $scope['sql'] . "
    ...
    public function selectDocumentosLegajoVigentes(array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $sql = "SELECT COUNT(*) AS cant_documentos_vigentes
                ...
                WHERE ld.estado = 'cargado'
                  AND l.estado NOT IN ('cerrado', 'aprobado')" . $scope['sql'];
        $res = $this->select($sql, $scope['params']);
        return $res;
    }
    ...
    public function selectDocumentosLegajoPorVencer(int $diasMargen = 30, array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $sql = "SELECT COUNT(*) AS cant_documentos_por_vencer
                ...
                INNER JOIN cfg_legajo l ON l.id_legajo = ld.id_legajo
                WHERE ld.estado = 'cargado'
                  AND l.estado NOT IN ('cerrado', 'aprobado')
    ...
    public function selectDocumentosLegajoCriticos(array $tiposPermitidos = [], int $idUsuario = 0, bool $soloPropios = false)
    {
        ...
        $sql = "SELECT COUNT(*) AS cant_documentos_criticos
                FROM cfg_legajo_documento ld
                INNER JOIN cfg_legajo l ON l.id_legajo = ld.id_legajo
                WHERE ld.estado IN ('pendiente', 'vencido')
                  AND l.estado NOT IN ('cerrado', 'aprobado')" . $scope['sql'];
        $res = $this->select($sql, $scope['params']);
        return $res;
    }
    ...
    public function selectPorcAvanza()
    {
        $sql = "SELECT 
                    CASE 
                        WHEN COALESCE(MAX(cfg.total_pag_objetivo), 0) <= 0 THEN 0
                        ELSE (COALESCE(SUM(a.total_pag), 0) / (MAX(cfg.total_pag_objetivo) * 100))
                    END AS porc_avanzar
                FROM detalle_proceso a
                CROSS JOIN (
                    SELECT COALESCE(MAX(total_pag), 0) AS total_pag_objetivo
                    FROM configuracion
                ) cfg";
        $res = $this->select($sql);
        // echo $res;
    ...
    {
        $sql = "SELECT 
                    COALESCE(MAX(cfg.total_pag_objetivo), 0) - COALESCE(SUM(a.total_pag), 0) AS cant_faltantes
                FROM detalle_proceso a
                CROSS JOIN (
                    SELECT COALESCE(MAX(total_pag), 0) AS total_pag_objetivo
                    FROM configuracion
    ...
    {
        $sql = "SELECT COUNT(*) AS cant_usuarios FROM visitas WHERE estado='ACTIVO'";
        $res = $this->select($sql);
        return $res;
    }
