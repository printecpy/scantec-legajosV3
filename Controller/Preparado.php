<?php
    class Preparado extends Controllers{
        ...
        {
            $preparar = $this->model->selectPreparado();
            $operador = $this->model->selectOperador();
            $usuario = $this->model->selectUsuarios();
            $data = ['preparar' => $preparar, 'usuario' => $usuario, 'operador' => $operador];
        ...
        public function registrar()
        {
            ...
            $insert = $this->model->insertarPreparado($fecha, $cant_expediente, $cant_cajas, $observaciones, $id, $id_operador);
            if ($insert) {
            header("location: " . base_url() . "preparado");
            die();    
            }
        }
        ...
        {
            $id_prep = $_GET['id_prep'];
            $preparar = $this->model->editPreparado($id_prep);
            $operador = $this->model->selectOperador();
            $usuario = $this->model->selectUsuarios();
            $data = ['preparar' => $preparar, 'operador' => $operador, 'usuario' => $usuario];
            if ($data == 0) {
                $this->preparado();
            } else {
        ...
        public function modificar()
        {
                ...
                $observaciones = $_POST['observaciones'];
                $id_operador = $_POST['id_operador'];
                $actualizar = $this->model->actualizarPreparado($fecha, $cant_expediente, $cant_cajas, $observaciones, $id_operador, $id_prep);
                if ($actualizar) {   
                    header("location: " . base_url() . "preparado"); 
                    die();
                }
        ...
        {
            $id_prep = $_POST['id_prep'];
            $this->model->estadoPreparado('INACTIVO', $id_prep);
            header("location: " . base_url() . "preparado");
            die();
        }
        ...
        {
            $id_prep = $_POST['id_prep'];
            $this->model->estadoPreparado('ACTIVO', $id_prep);
            header("location: " . base_url() . "preparado");
            die();
        }
        ...
        public function pdf()
        {
            ...
            $preparado = $this->model->selectPreparado();
            require_once 'Libraries/pdf/fpdf.php';
            require_once 'Config/ConfigPath.php';
            $pdf = new FPDF('L', 'mm', 'A4');
            $pdf->AddPage();
            $pdf->SetTitle('Preparado de expedientes');
            ...
            $pdf->Cell(280, 8, "Preparado de expedientes", 1, 1, 'C', 1);
            $pdf->Ln();
            $pdf->setX(45);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetFillColor(192, 192, 192);
            ...
            $pdf->Cell(30, 5, utf8_decode('Cant. expediente'), 1, 0, 'C',1);
            ...
            $pdf->SetFont('Arial', '', 8);
            foreach ($preparado as $row) {
                $pdf->setX(13);
                $pdf->Cell(15, 5, $row['id_prep'], 1, 0, 'C');
                $pdf->Cell(30, 5, utf8_decode($row['fecha']), 1, 0, 'C');
                $pdf->Cell(30, 5, number_format($row['cant_expediente'], 0, ',', '.'), 1, 0, 'C');
            ...
            $pdf->Output("Preparado_" . date('Y_m_d_H_i_s').".pdf", "D");
        }
        ...
        public function pdf_filtroFecha()
        {
            ...
            $hasta = $_POST['hasta'];
            $datos = $this->model->selectDatos();
            $preparado = $this->model->reportePreparadoFecha($desde, $hasta);
            require_once 'Libraries/pdf/fpdf.php';
            require_once 'Config/ConfigPath.php';
            $pdf = new FPDF('L', 'mm', 'A4');
            $pdf->AddPage();
        ...
        public function pdf_filtroTotal()
        {
            ...
            $datos = $this->model->selectDatos();
            $preparado = $this->model->reportePreparadototal($mes_desde, $anio_desde, $mes_hasta, $anio_hasta);
            require_once 'Libraries/pdf/fpdf.php';
            require_once 'Config/ConfigPath.php';
            ...
            $pdf->SetFillColor(192, 192, 192);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->Cell(195, 8, "Preparado de expedientes", 1, 1, 'C', 1);
            $pdf->Ln();
            $pdf->setX(45);
            $pdf->SetTextColor(0, 0, 0);
            $pdf->SetFillColor(192, 192, 192);
            ...
            $pdf->Cell(34, 6, utf8_decode('Cant expedientes'), 1, 0, 'C',1);
        ...
        public function pdf_filtroOperador()
        {
            $id_operador = $_POST['id_operador'];
            $datos = $this->model->selectDatos();
            $preparado = $this->model->reportePreparadoOperador($id_operador);
            require_once 'Libraries/pdf/fpdf.php';
            require_once 'Config/ConfigPath.php';
            $pdf = new FPDF('P', 'mm', 'A4');
            $pdf->AddPage();
        ...
        public function excel()
        {
            ...
            $spreadsheet->getProperties()
                ...
                ->setTitle('Preparado de expedientes')  
                ->setSubject('Reporte de preparado')
                ->setDescription('Reporte de expedientes y cajas preparadas')
                ->setKeywords('preparados, expedientes, reporte')
                ->setCategory('Reporte');  
            // Establecer estilo de fuente predeterminado
            $spreadsheet->getDefaultStyle()->getFont()->setName('Helvetica Neue'); 
            $sheet->setCellValue('A1', 'ID');
            $sheet->setCellValue('B1', 'FECHA');
            $sheet->setCellValue('C1', 'CANT EXPEDIENTE');
            ...
            $row = 2;    
            foreach ($preparado as $value) {
                $sheet->setCellValue('A'.$row, $value["id_prep"]);
                $sheet->setCellValue('B'.$row, $value["fecha"]);
                $sheet->setCellValue('C'.$row, $value['cant_expediente']); 
        ...
        public function excel_filtroTotal()
        {
            ...
            $sheet->setCellValue('B1', 'CANT EXPEDIENTES');
            ...
            foreach ($preparado as $value) {
                ...
                $sheet->setCellValue('B'.$row, $value["cant_expedientes"]);
